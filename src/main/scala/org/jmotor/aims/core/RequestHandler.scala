  ...
  override def receive: Receive = {
    case request: ServiceRequest ⇒
      handle(request.request) match {
        case unit: BoxedUnit        ⇒ request.original ! HttpResponse(200)
        case response: HttpResponse ⇒ request.original ! response
        case string: String         ⇒ request.original ! HttpResponse(200, entity = string)
        case entity                 ⇒ request.original ! HttpResponse(200, entity = entity.toString)
      }
